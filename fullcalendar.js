...
var defaults = {
	...
	timeline_settings:  {
		...
		datesDiv: 					'#dates',			// value: any HTML tag or #id, default to #datesded), default to 500 (normal)
    ...
    event_type_class: {default:'fc-event'}
...
function Calendar(element, options, eventSources, resourceSources) {
	...
	function _renderView(inc) { // assumes elementVisible
		...
		trigger('viewComplete', null, true);
	...
	function rerenderEvents(modifiedEventID) { // can be called as an API method
		console.log('rerenderEvents -> modifiedEventID: ' + modifiedEventID);
    clearEvents();
	...
	function updateTodayButton() {
		...
		if (today >= currentView.start && today < currentView.end) {
			if(currentView.name === 'resourceWeek' || currentView.name === 'resourceMonth') {
...
function Header(calendar, options) {
	...
	function render() {
		...
		if(options.timeline && options.defaultView=='resourceDay'){
			if (sections) {
				$('.fc-header').remove();
				element = $("<table class='fc-header' style='width:100%;margin-bottom: 8px !important;'/>")
					.append(
						$("<tr/>")
							.append(renderSection('timeline'))
					);
				return element;
			}
		}else{
    ...
    function buildTimelineDates(date){
        var liHtml = '';
        var currentDate = new Date();
        var curDateStr = currentDate.getDate()+''+currentDate.getMonth()+''+currentDate.getFullYear();
        var dayAlreadySelected = false;
        for(i=0;i<options.timeline_settings.howManyDates;i++){
			var d = date.getDate();
			var spanClazz = d==1?'firstMonthDay':'';
			var m = options.timeline_settings.months[date.getMonth()];
			var weekday = options.timeline_settings.weekdays[date.getDay()];
			var liWeekend = '';
			if(options.timeline_settings.skipWeekend){
				liWeekend = weekday=='SAT'||weekday=='SUN'?'dateDisabled':'';
			}
            var dateStr = date.getDate()+''+date.getMonth()+''+date.getFullYear();
			var todayHtml = curDateStr==dateStr?'<div class="today">TODAY</div>':'';
			var liClazz = curDateStr==dateStr?'today':'';
            if(i>=0 && i<=2 && liWeekend!='dateDisabled' && !dayAlreadySelected){//this is to avoid selecting a disabled day
                liClazz = liClazz + ' selected';
                dayAlreadySelected = true;
            }
			liHtml = liHtml + '<li class="'+liClazz+' '+liWeekend+'" date="'+date+'">'
				+'<a href="#">'+d+'</a>'
				+'<span class="'+spanClazz+'" style="display:none;">'+m+'</span>'
				+'<div style="display:none;">'+weekday+'</div>'
				+todayHtml
				+'</li>';
	...
	function renderTimeline(sectionEl){
		var widthContainer = $(options.timeline_settings.parent).width()*0.96;
		options.timeline_settings.howManyDates =  parseInt(widthContainer/20) - 1; //20 is the li width
		
		//creating dates
		var datesHtml = '<ul id="dates"><div class="datesWrapper">';
		var date = new Date();
		var liHtml = buildTimelineDates(date);
		datesHtml = datesHtml + liHtml + '</div></ul><a href="#" id="'+options.timeline_settings.nextButton+'"></a><a href="#" id="'+options.timeline_settings.prevButton+'"></a>';
		sectionEl.html(datesHtml);
		
		$(sectionEl.children().children().children('li:not(.dateDisabled)')).click(function(event){
			event.preventDefault();
			$(options.timeline_settings.datesDiv+' li').removeClass('selected');
			$(this).addClass('selected');
			var date = new Date($(this).attr('date'));
			calendar.gotoDate(date);
		});

		$(sectionEl.children('#'+options.timeline_settings.nextButton)).bind('click', function(event){
			event.preventDefault();
			// bugixed from 0.9.54: now the dates gets centered when there's too much dates.
			var date = new Date($(options.timeline_settings.datesDiv+' li').last().attr('date'));
            date.setDate(date.getDate() + 1);
            var liHtml = buildTimelineDates(date);
			$(options.timeline_settings.datesDiv+' li').remove();
            $(options.timeline_settings.datesDiv + ' .datesWrapper').append(liHtml);
            
            //binding click event
            $(options.timeline_settings.datesDiv+' li:not(.dateDisabled)').click(function(event){
                event.preventDefault();
                $(options.timeline_settings.datesDiv+' li').removeClass('selected');
                $(this).addClass('selected');
                var date = new Date($(this).attr('date'));
			...
			date = new Date($(options.timeline_settings.datesDiv).find('li.selected').attr('date'));
            //Call the Availability method and pass the date variable
			calendar.gotoDate(date);
		});

		$(sectionEl.children('#'+options.timeline_settings.prevButton)).click(function(event){
			event.preventDefault();
			// bugixed from 0.9.54: now the dates gets centered when there's too much dates.
			var date = new Date($(options.timeline_settings.datesDiv+' li').first().attr('date'));
			date.setDate(date.getDate() - options.timeline_settings.howManyDates);
            var liHtml = buildTimelineDates(date);
			$(options.timeline_settings.datesDiv+' li').remove();
            $(options.timeline_settings.datesDiv + ' .datesWrapper').append(liHtml);
		...
		// keyboard navigation
		if(options.timeline_settings.arrowKeys=='true') {
			$(document).keydown(function(event){
				if (event.keyCode == 39) { 
				   $('#'+options.timeline_settings.nextButton).click();
				}
				if (event.keyCode == 37) { 
				   $('#'+options.timeline_settings.prevButton).click();
				}
			});
		}
	}
...
function ResourceManager(options) {
    ...
    var cache;  // cached resources
      ...
      normalizeSource(resource);
      source = {
        resources: resource
      };
      
      sources.push(source);
      t.getView().getResources.push(resource);
...
*/
	...
	function buildFootHTML() {
		...
		html += "<tfoot>"
			+ "<tr class='fc-footer-heigt'>"
			+ "<td class='fc-footer fc-footer-cstm'>" 
			+ "<div class='addName' onclick='angular.element(\"[ng-controller="+opt('controller')+"]\").scope()."+opt('clickAdd')+";'>Click to add attendee</div>"
			+ "</td>";
		
		for (col=0; col<colCnt; col++) {
			html += "<td class='fc-footer-td'></td>" ;
		}
			+ "</tr>"
			+ "</tfoot>";
...
function ResourceEventRenderer() {
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
            ...
            var clazz = '';
            clazz = t.calendar.options.event_type_class[event.status];
