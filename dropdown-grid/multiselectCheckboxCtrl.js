var myapp = angular.module('grid-select', ['ng']);
myapp.directive( 'gridSelect' , [ '$sce', '$timeout', function ( $sce, $timeout ) {
  return {
        restrict: 
            'AE',
                ...
                '<div class="checkboxLayer">' +                        
                    '<form>' + 
                        '<div  class=" row helperContainer">' +
                                '<div class="line col-lg-9" style="position:relative" ng-if="displayHelper( \'filter\' )">' +
                                ...
                                '</div>' +
                                '<div class="col-lg-3 btn-group"><button type="button" class="button">Action</button>'+
                                '</div>'+
                        '</div>' +
                        '<div class="checkBoxContainer" style="{{setHeight();}}">' +
                            ...
                            '<table class="table">'+
                            '<tr>'+
                            '<th ng-repeat="item in columnNames"><span ng-bind-html="writeHeadColumn( item )"></span></th>'+
                            '</tr>'+
                            '<tr ng-repeat="data in inputModel2" ng-click="syncItemsGrid(data.id,data,$event,$index);" ng-class="{selected:data.id===idSelected}">'+
                            '<td><span ng-bind-html="writeData(data.name)"></span></td><td><span ng-bind-html="writeData(data.title)"></span></td>'+
                            '<td><span ng-bind-html="writeData(data.organization)"></span></td><td><span ng-bind-html="writeData(data.bussines)"></span></td>'+
                            '</tr>'+
                            '</table>'+
                            <!-- END TABLE -->
                            '<div ng-repeat="item in filteredModel | filter:removeGroupEndMarker" class="multiSelectItem"' +
                                ...
                                '<span class="tickMark" ng-if="item[ groupProperty ] !== true && item[ tickProperty ] === true"><i class="glyphicon glyphicon-ok"></i></span>' +
                                '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + 
                                '<div class="acol">' +
                                    '<label>' +
        ...
        link: function ( $scope, element, attrs ) {           
            ...
            $scope.syncItemsGrid = function(id,item, event, ng_repeat_index){
              $scope.idSelected=id;
              index = $scope.inputModel2.indexOf( item );
              for( i=0 ; i < $scope.inputModel2.length ; i++) {                            
                   $scope.inputModel2[ i ][ $scope.tickPropertyGrid ] = false;                            
                 }        
               $scope.inputModel2[ index ][ $scope.tickPropertyGrid ] = true; 
            ...
            $scope.refreshHeader = function(){
                $scope.headerName=$scope.columnNames;
            };
            ...
            $scope.writeHeadColumn = function(item){
                var label='';
            ...
            $scope.$watch('inputModel2',function(newVal){
                if(newVal){
                    console.log(newVal);
...
myapp.controller('gridSelectCtrl', function ($scope) {
   $scope.showHelper=true; //disable helper container
